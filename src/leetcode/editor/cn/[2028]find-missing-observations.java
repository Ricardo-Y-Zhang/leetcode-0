...
class Solution {
    public int[] missingRolls(int[] rolls, int mean, int n) {
        ...
        int laverage = (int)Math.floor((double)need/n);
        int raverage = (int)Math.ceil((double)need/n);
        if (laverage<1 || raverage > 6) {
            return new int[0];
        }
        if (laverage == raverage) {
            Arrays.fill(ans, laverage);
        }else{
            for (int i = 0; i < raverage*n - need; i++) {
                ans[i] = laverage;
            }
            for (int i = raverage*n-need; i < n; i++) {
                ans[i] = raverage;
            }
        }
