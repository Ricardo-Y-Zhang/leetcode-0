...
// 
// Related Topics 深度优先搜索 广度优先搜索 并查集 图 👍 354 👎 0
...
class Solution {
    ...
    public boolean isBipartite(int[][] graph) {
        ...
        color = new int[n];
        for (int i = 0; i < n; i++) {
            if (color[i] == 0){//深度优先搜索未被染色的节点
                if (!dfs(graph, i, 1)) return false;
            }
        }
        return true;
    }
    public boolean dfs(int[][] graph, int index, int col) {//节点index应被染成 col 颜色
        if (flag) return false;
        color[index] = col;
        for (int next : graph[index]){
            if (color[next] == col) {//邻接点与index节点颜色相同
                flag = true;
                return false;
            }
            if (color[next] == 0) {//未染色即未访问
                if(!dfs(graph, next, 3-col)) {//将next节点染成 3-col 颜色
                    return false;
                }
            }
        }
