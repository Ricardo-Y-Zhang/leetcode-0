...
class Solution {
    ...
    public int find(String answerKey, int k, char ch) {
        int l = 0, r = 0, n = answerKey.length();
        int ans = 0;
        //保证 [l,r] 中非 ch 字符个数不超过 k
        while (r < n) {
            char temp = answerKey.charAt(r);
            if (temp != ch) {
                k--;
                if (k < 0) {//超过 k 个，将一个非 ch 移出窗口
                    while (l<n&&answerKey.charAt(l) == ch) {
                        l++;
                    }
                    l++;
                }
